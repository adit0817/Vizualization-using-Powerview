...
# Open the CSV file
reader = csv.reader(open('C:\Stuff\Study\Sem 2\Data Visualization\Assignments\Assignment 4\Largest_Cities_CSV.csv'), delimiter="," )
...
# Populate the values for the columns
					for j in range(0, 4):
						if item[i+2] is not '':
							if j==0:
#Concatenate the State and the City value
								r[0] = item[j+1]+' - '+item[j]
								pool.append([])
								pool[a-1].append(r[0])
# Populate Year Value
...
# Initialize the rank to 0, rank are calculated later in the program
							if j ==3:
								pool[a-1]. append(int(0))
	rows_so_far +=1
...
# Populate the list with all the values except header as
# Sorting cannot be done on the integer values when one record has character value
list = pool[1:a-1]
...
mycsv = csv.writer(open('C:\Stuff\Study\Sem 2\Data Visualization\Assignments\Assignment 4\Adit_Kansara.csv', 'wb'))
for row in list1:
...
# if same calculate that the rank, if different assign the rank as one
	if row[1] <> c and e <> 0:
		v = 1
		c = row[1]
		row[3] = v
